...
perEachTestConfig(({ outputDTsFiles, outputFile }) => {
    describe(`Projection Entities Tests (using output **/*/${outputFile} files)`, () => {
        let astw

        before(async () => {
            configuration.outputDTsFiles = outputDTsFiles
            astw = (await prepareUnitTest('projection/model.cds', locations.testOutput('projection_test'))).astw
        })
