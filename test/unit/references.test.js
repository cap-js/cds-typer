...
describe('References', () => {
    ...
    test('Entity', async () => {
        ...
        expect(astw.exists('_BarAspect', 'assoc_many', m => true
                && m.type.name === 'many'
        ...
        expect(astw.exists('_BarAspect', 'comp_many', m => true
                && m.type.name === 'many'
                && m.type.args[0].name === 'Foo_'
        )).toBeTruthy()
        expect(astw.exists('_BarAspect', 'assoc_one_first_key', m => check.isNullable(m.type, [check.isString]))).toBeTruthy()
        expect(astw.exists('_BarAspect', 'assoc_one_second_key', m => check.isNullable(m.type, [check.isString]))).toBeTruthy()
        expect(astw.exists('_BarAspect', 'assoc_one_ID', m => check.isNullable(m.type, [check.isString]))).toBeTruthy()
    })
