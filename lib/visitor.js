...
class Visitor {
    ...
    #staticClassContents(clean, entity) {
        return Object.hasOwn(entity, '@fiori.draft.enabled') ? [`static drafts: typeof ${clean}`] : [] 
    }
    ...
    #printEntity(name, entity) {
        ...
        buffer.add(`export class ${plural} extends Array<${singular}> {${this.#staticClassContents(clean, entity).join('\n')}}`)
        buffer.add('')
    }
