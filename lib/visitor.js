...
class Visitor {
     ...
     * @param {{cleanName?: string, excluding?: string[], renames?: {[key: string]: string}}} options - `excluding` removes certain elements by name. If `renames` is passed, it supersedes `excluding`!
    ...
    #aspectify(name, entity, buffer, options = {}) {
        options = { ...{ 
            ...
            renames: Object.fromEntries(entity?.projection?.columns?.map(({ref, as}) => [ref[0], as]) ?? [])
        }, ...options }
        ...
        buffer.addIndentedBlock(`export function ${identAspect(clean)}<TBase extends new (...args: any[]) => object>(Base: TBase) {`, function () {
            buffer.addIndentedBlock(`return class ${clean} extends Base {`, function () {
                ...
                const relevantElements = Object.keys(options.renames).length
                    ? elements.filter(([ename]) => options.renames[ename]).map(([ename, element]) => [options.renames[ename], element])
                    : elements.filter(([ename]) => !exclusions.has(ename))

                for (let [ename, element] of relevantElements) {
                    if (options.renames?.[ename]) {
                        ename = options.renames[ename]
                    }
