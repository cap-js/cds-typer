...
const { isBuiltinType } = require('./resolution/type-guards')
...
class Visitor {
                    ...
                    if (typeof e?.type !== 'string' && e?.type?.ref) {
                        ...
                        try {
                            ...
                            for (let i = 0; i < MAX_TRANSITIVE_RESOLUTION_STEPS; i++) {
                                const { csn } = this.resolver.resolveByTypeName(/** @type {string} */(e.resolvedType))
    ...
    #printType(fq, type) {
        ...
        if (isEnum(type) && !isReferenceType(type) && this.resolver.builtinResolver.resolveBuiltin(type.type)) {
            ...
            if (configuration.brandedPrimitiveTypes && isBuiltinType(resolved.typeInfo)) {
