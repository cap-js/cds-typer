...
class Visitor {
     ...
     * @param {{cleanName?: string, excluding: string[]}} options
    ...
    #aspectify(name, entity, buffer, options = {}) {
        options = { ...{ cleanName: undefined, excluding: entity?.projection?.excluding }, ...options }
    ...
    #printEntity(name, entity) {
       ...
       let { singular, plural } = this.resolver.inflect({csn: entity, plainName: clean}, ns.asNamespace())
        ...
        const target = isProjection(entity)
            // prefer to look up in xtended -> we have draftability denoted there and only the neccessary properties. Nested elements (E.X.Y) do not exist there and therefore have to be looked up in inferred.
            ? (this.csn.xtended.definitions[getProjectionTarget(entity)] ?? this.csn.inferred.definitions[getProjectionTarget(entity)])
            : entity
