...
class Visitor {
     ...
     * @param {string[]} ancestors - the fully qualified names of the ancestors of the entity
    ...
    #printStaticActions(entity, buffer, ancestors, file) {
        ...
        const inherited = ancestors.map(a => `typeof ${a}.actions`).join(' & ')
        if (actions.length) {
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        const ancestors = (entity.includes ?? [])
            .map(ancestor => {
                ...
                return toLocalIdent({ns: namespace, clean: entityName, fq: ancestor})
            })

        const ancestorsAspects = ancestors
            .reverse() // reverse so that own aspect A is applied before extensions B,C: B(C(A(Entity)))
            .reduce((wrapped, ancestor) => `${identAspect(ancestor)}(${wrapped})`, 'Base')
        ...
        buffer.addIndentedBlock(`export function ${identAspect(clean)}<TBase extends new (...args: any[]) => object>(Base: TBase) {`, () => {
            buffer.addIndentedBlock(`return class ${clean} extends ${ancestorsAspects} {`, () => {
                ...
                buffer.addIndented(() => {
                    ...
                    this.#printStaticActions(entity, buffer, ancestors, file)
                })
