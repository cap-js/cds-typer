...
const { warn, error, debug } = require('./logging')
...
class Visitor {
    ...
    visitDefinitions() {
        for (const [name, entity] of Object.entries(this.csn.xtended.definitions)) {
            if (isView(entity)) {
                ...
                warn(`Skipping unresolved entity: ${name}`)
            }
        }
        // FIXME: optimise
        ...
        for (const [name, entity] of missing) {
            ...
            if (entity.projection) {
                ...
                error(`Expecting an autoexposed projection within a service. Skipping ${name}`)
            }
        }
    }
            ...
            buffer.addIndentedBlock(`return class ${clean} extends Base {`, function () {
                ...
                for (let [ename, element] of Object.entries(entity.elements ?? {})) {
                    if (element.target && /\.texts?/.test(element.target)) {
                        warn(`referring to .texts property in ${name}. This is currently not supported and will be ignored.`)
                        continue
                    }
                        ...
                        for (const [kname, originalKeyElement] of this.#keys(element.target)) {
                            if (this.resolver.getMaxCardinality(element) === 1 && typeof element.on !== 'object') {  // FIXME: kelement?
                                ...
                                if (Object.hasOwn(entity.elements, foreignKey)) {
                                    error(`Attempting to generate a foreign key reference called '${foreignKey}' in type definition for entity ${name}. But a property of that name is already defined explicitly. Consider renaming that property.`)
                                } else {
    ...
    visitEntity(name, entity) {
        ...
        case 'service':
            ...
            break
        default:
            debug(`Unhandled entity kind '${entity.kind}'.`)
        }
    }
