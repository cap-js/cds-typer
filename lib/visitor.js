...
class Visitor {
    ...
    #printOperation(fq, operation, kind) {
        ...
        const ns = this.resolver.resolveNamespace(fq)
        const file = this.fileRepository.getNamespaceFile(ns)
    ...
    #printType(fq, type) {
        ...
        const { namespace: ns, name: clean } = this.resolver.untangle(fq)
        const file = this.fileRepository.getNamespaceFile(ns)
        ...
        if ('enum' in type && !isReferenceType(type) && this.resolver.builtinResolver.resolveBuiltin(type.type)) {
            file.addEnum(fq, clean, csnToEnumPairs(type))
        } else {
            // alias
            file.addType(fq, clean, this.resolver.resolveAndRequire(type, file).typeName)
        }
    ...
    #printAspect(fq, aspect) {
        ...
        file.addClass(clean, fq)
        file.aspects.add(`// the following represents the CDS aspect '${clean}'`)
        this.#aspectify(fq, aspect, file.aspects, { cleanName: clean })
    }
