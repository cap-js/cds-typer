...
const { isView, isUnresolved, propagateForeignKeys, collectDraftEnabledEntities, isDraftEnabled, isType, isProjection, getMaxCardinality, isViewOrProjection, isEnum, isEntity, isStructuredType } = require('./csn')
...
class Visitor {
    ...
    visitElement({name, element, file, buffer = file.classes, resolverOptions}) {
        // always flatten structured types
        const resolver = isStructuredType(this.entityRepository.getByFq(/** @type {string}*/(element.type))?.csn)
            ? new FlatInlineDeclarationResolver(this)
            : this.inlineDeclarationResolver

        return resolver.visitElement({
            name,
            element,
            file,
            buffer,
