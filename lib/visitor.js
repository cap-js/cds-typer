...
const { docify, promiseOf, unionOf, stringIdent, intersectionOf, brandedType } = require('./printers/wrappers')
...
class Visitor {
    ...
    constructor(csn) {
        ...
        configuration.outputDTsFiles=true
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        const ancestorsAspects = ancestorInfos
            .toReversed() // reverse so that own aspect A is applied before extensions B,C: B(C(A(Entity)))
            .reduce((wrapped, ancestor) => `${asAspectFnIdentifier({info: ancestor, wrapper: identAspect, relative: file.path})}(${wrapped})`, 'Base')
        ...
        for (const e of enums) {
            ...
            const enumName = propertyToInlineEnumName(clean, normalizedPropertyName)
    ...
    #printEvent(fq, event) {
        ...
        buffer.add(this.tsPrinter.printSingleLineComment('event'))
        ...
        buffer.addIndentedBlock(`export declare class ${entityName} {`, () => {
            // In .d.ts files, events already use declare class, so kind doesn't need initializer
