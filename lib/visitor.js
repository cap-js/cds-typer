...
const { amendCSN, isView, isUnresolved, propagateForeignKeys, isDraftEnabled, isType, isProjection, getProjectionTarget, getProjectionRenames } = require('./csn')
...
const { getProjectionAliases } = require('./csn')
...
class Visitor {
     ...
     * @param {{cleanName?: string, excluding?: string[], renames?: {[key: string]: string}}} options - `excluding` removes certain elements by name. If `columns` is passed, it supersedes `excluding`!
    ...
    #aspectify(name, entity, buffer, options = {}) {
        options = { ...{ 
            cleanName: undefined,
            excluding: entity?.projection?.excluding,
            columns: {}
        }, ...options }
        ...
        buffer.addIndentedBlock(`export function ${identAspect(clean)}<TBase extends new (...args: any[]) => object>(Base: TBase) {`, function () {
            buffer.addIndentedBlock(`return class ${clean} extends Base {`, function () {
                const enums = []
                const exclusions = new Set(options.excluding ?? [])
                const elements = Object.entries(entity.elements ?? {})
                const relevantElements = Object.keys(options.columns?.aliases ?? {}).length
                    ? elements.filter(([ename]) => options.columns?.aliases[ename]).map(([ename, element]) => [options.columns?.aliases[ename][0], element])
                    : elements.filter(([ename]) => !exclusions.has(ename))

                for (let [ename, element] of relevantElements) {
                    const aliases = options.columns?.aliases?.[ename] ?? []
                    if (options.columns?.all) this.logger.warning(`selecting all elements (*) within a projection is currently not supported and will be ignored.`)
                    if (aliases.length > 1) this.logger.warning(`multiple aliases for ${ename} in ${name}. This is currently not supported and will be ignored.`)
                    if (aliases.length) ename = aliases[0]
    ...
    #printEntity(name, entity) {
       ...
       let { singular, plural } = this.resolver.inflect({csn: entity, plainName: clean}, ns.asNamespace())
        ...
        // in case of projections `entity` is empty -> print all of the projection target's elements instead
        const target = isProjection(entity)
            ? this.csn.inferred.definitions[getProjectionTarget(entity)]
            : entity
        ...
        this.#aspectify(name, target, buffer, {
            cleanName: singular,
            excluding: entity?.projection?.excluding,
            columns: getProjectionAliases(entity)
        })
