...
class Visitor {
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        // CLASS ASPECT
        buffer.addIndentedBlock(this.tsPrinter.printAspectFunction(aspectFunctionName), () => {
            //------
            ...
            buffer.addIndentedBlock(this.tsPrinter.printAspect(clean, ancestorsAspects), () => {
                ...
                if ('kind' in entity) {
                    ...
                    buffer.add(createMember({
                        name: 'kind',
                        type: configuration.outputDTsFiles
                            ? stringIdent(entity.kind)
                            : createUnionOf(...['entity', 'type', 'aspect'].map(stringIdent)),
                        isStatic: true,
                        isReadonly: true,
                        isDeclare: false,
                        isOverride: ancestorInfos.some(({csn}) => csn.kind),
                        initialiser: configuration.outputDTsFiles ? undefined : stringIdent(entity.kind)
                    }))
                }
