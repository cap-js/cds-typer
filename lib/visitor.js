...
const { csnToEnum, propertyToEnumName } = require('./printers/enum')
...
class Visitor {
    ...
    #aspectify(name, entity, buffer, cleanName = undefined) {
        ...
        for (const [ename, element] of Object.entries(entity.elements ?? {})) {
            ...
            if ('enum' in element && !(element.type in this.csn.definitions)) {
                const enumName = propertyToEnumName(clean, element.name)
                file.addInlineEnum(enumName, csnToEnum(element))
                buffer.add(`static ${element.name} = { enum: ${enumName} }`)
            }
        }      
