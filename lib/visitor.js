...
const { amendCSN, isView, isUnresolved, propagateForeignKeys, isDraftEnabled, isType, isProjection, getMaxCardinality, isViewOrProjection, isEnum, isEntity, isStructuredType } = require('./csn')
...
class Visitor {
    ...
    visitElement({name, element, file, buffer = file.classes, resolverOptions}) {
        ...
        const resolver = isStructuredType(this.entityRepository.getByFqOrThrow(/** @type string*/(element.type)).csn)
            ? new FlatInlineDeclarationResolver(this)
            : this.inlineDeclarationResolver
