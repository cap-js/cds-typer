...
const { EntityRepository } = require('./resolution/repository')
...
class Visitor {
            ...
            buffer.addIndentedBlock(`return class ${clean} extends Base {`, function () {
                ...
                for (let [ename, element] of Object.entries(entity.elements ?? {})) {
                    if (element.target && /\.texts?/.test(element.target)) {
                        this.logger.warning(`referring to .texts property in ${fq}. This is currently not supported and will be ignored.`)
                        continue
                    }
                    ...
                    if ('target' in element) {
                        ...
                        for (const [kname, kelement] of this.#keys(element.target)) {
                            if (getMaxCardinality(element) === 1) {  // FIXME: kelement?
                                const foreignKey = `${ename}_${kname}`
                                if (Object.hasOwn(entity.elements, foreignKey)) {
                                    this.logger.error(`Attempting to generate a foreign key reference called '${foreignKey}' in type definition for entity ${fq}. But a property of that name is already defined explicitly. Consider renaming that property.`)
                                } else {
    ...
    #printOperation(fq, operation, kind) {
        this.logger.debug(`Printing operation ${fq}:\n${JSON.stringify(operation, null, 2)}`)
        ...
        file.addOperation(fq.split('.').at(-1), params, returns, kind)
    }

    #printType(fq, type) {
        this.logger.debug(`Printing type ${fq}:\n${JSON.stringify(type, null, 2)}`)
    ...
    #printAspect(fq, aspect) {
        this.logger.debug(`Printing aspect ${fq}`)
        const { namespace, entityName } = this.entityRepository.getByFq(fq)
        const file = this.fileRepository.getNamespaceFile(namespace)
    ...
    #printEvent(fq, event) {
        this.logger.debug(`Printing event ${fq}`)
        const { namespace, entityName } = this.entityRepository.getByFq(fq)
        const file = this.fileRepository.getNamespaceFile(namespace)
        ...
        buffer.addIndentedBlock(`export declare class ${entityName} {`, function() {
    ...
    #printService(fq, service) {
        this.logger.debug(`Printing service ${fq}:\n${JSON.stringify(service, null, 2)}`)
        const { namespace } = this.entityRepository.getByFq(fq)
        const file = this.fileRepository.getNamespaceFile(namespace)
