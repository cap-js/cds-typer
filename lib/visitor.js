...
const { amendCSN, isView, isUnresolved, propagateForeignKeys, isDraftEnabled, isType, isProjection, getMaxCardinality, isViewOrProjection } = require('./csn')
...
const { EntityRepository, asAspectIdentifier, asIdentifier } = require('./resolution/entity')
...
class Visitor {
    ...
    /**
     * @param {CSN} entity - the entity to print the actions for
     ...
     * @param {File} file - the file the entity is being printed into
     */
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        /** @param {import('./typedefs').resolver.EntityInfo} name */
        const identAspect = name =>  `_${name}Aspect`
        /**
        ...
        const ancestorInfos = ((!isViewOrProjection(entity) ? entity.includes : []) ?? [])
            .map(ancestor => {
                //const { namespace, entityName } = this.entityRepository.getByFq(ancestor)
    ...
    #printType(fq, type) {
        ...
        if ('enum' in type && !isReferenceType(type) && this.resolver.builtinResolver.resolveBuiltin(type.type)) {
            ...
            const isEnumReference = this.csn.inferred.definitions[type?.type]?.enum
            // alias
    ...
    /**
     * @param {string} fq
     * @param {{ name: string; doc?: string; operations?: {returns: any }}} service
     */
    #printService(fq, service) {
        ...
        this.#renderDoc(service.doc, buffer)
        // file.addImport(new Path(['cds'], '')) TODO make sap/cds import work
        buffer.addIndentedBlock('export default class extends cds.Service {', () => {
            Object.entries(service.operations||{}).forEach(([name, {doc}]) => {
                this.#renderDoc(doc, buffer)
    ...
    /**
     * @param {string|undefined} doc - the documentation to add
     * @param {import('./file.js').Buffer} buffer - the buffer to add the documentation to
     */
    #renderDoc(doc, buffer = new Buffer()) {
        if (!doc) return ''
        buffer.add('/**')
        doc.split(/\r?\n/).forEach(line => { buffer.add(` * ${line}`) })
        buffer.add(' */')
        return buffer.join()
    }
