...
class Visitor {
    ...
    constructor(csn) {
        ...
        // FIXME: only while developing TSPrinters!!!!
        configuration.outputDTsFiles = true
        /** @type {import('./printers/typescript').Printer} */
        this.tsPrinter = new DeclarationPrinter() //new ImplementationPrinter()
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        // CLASS ASPECT
        buffer.addIndentedBlock(this.tsPrinter.printAspectFunction(aspectFunctionName, !!buffer.namespace), () => {
            //------
            const innerBuffer = buffer.createSubBuffer()
            ...
            for (let [ename, element] of Object.entries(entity.elements ?? [])) {
                ...
                this.visitElement({name: ename, element, file, buffer: innerBuffer, resolverOptions})
                    ...
                    for (const [kname, originalKeyElement] of this.#keys(element.target)) {
                        if (getMaxCardinality(element) === 1 && typeof element.on !== 'object') {  // FIXME: kelement?
                            ...
                            if (entity.elements && Object.hasOwn(entity.elements, foreignKey)) {
                                ...
                                this.visitElement({name: foreignKey, element: kelement, file, buffer: innerBuffer, resolverOptions})
                            }
                        }
                    }
                }
            ...
            for (const e of enums) {
                ...
                file.addInlineEnum(clean, fq, normalizedPropertyName, csnToEnumPairs(e, {unwrapVals: true}), innerBuffer, eDoc)
            }

            if ('kind' in entity) {
                // In .d.ts files, use literal type without initializer
                staticMembersBuffer.add(this.tsPrinter.printKindProperty(entity, ancestorInfos))
            }
            //buffer.add(innerBuffer.join('\n'))
            //-------

            buffer.addIndentedBlock(this.tsPrinter.printAspect(clean, innerBuffer, staticMembersBuffer, ancestorsAspects), () => {
