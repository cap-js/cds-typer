...
const { getPluralAnnotation, getSingularAnnotation } = require('./util')
...
class Visitor {
    ...
    #printType(fq, type) {
        ...
        // Honour @singular/@plural annotations; fall back to entityName when absent
        const singularAnnotation = getSingularAnnotation(type)
        const pluralAnnotation = getPluralAnnotation(type)
        const effectiveName = singularAnnotation ?? entityName

        // Normalize the effective name to handle reserved words and special characters
        const identifier = new Identifier(effectiveName)
        const normalizedName = identifier.normalised.plain
        ...
        if (isEnum(type) && !isReferenceType(type) && this.resolver.builtinResolver.resolveBuiltin(type.type)) {
            ...
            // Emit an explicit plural alias when @plural is set
            if (pluralAnnotation) {
                const pluralIdentifier = new Identifier(pluralAnnotation)
                file.addType(fq, pluralIdentifier.normalised.plain, `${normalizedName}[]`, isEnumReference)
                if (pluralIdentifier.normalised.isChangedFromNormalisation) {
                    file.types.add(`export { ${pluralIdentifier.normalised.plain} as "${pluralIdentifier.plain}" }`)
                }
            }
        }
    }
