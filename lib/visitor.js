...
const { isView, isUnresolved, propagateForeignKeys, collectDraftEnabledEntities, isDraftEnabled, isType, isProjection, getMaxCardinality, isViewOrProjection, isEnum } = require('./csn')
...
class Visitor {
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        // DRAFT CLASSES
        this.#printDraftClasses(fq, buffer, identSingular(clean), identAspect(clean))
    ...
    /**
     * Prints draft classes for draft enabled
     * @param {string} fq - FQN of entity
     * @param {Buffer} buffer - buffer to write exported drafts classes to
     * @param {string} singular - singular name of parent entity
     * @param {string} aspectFunction - class creator function of parent entity
     */
    #printDraftClasses(fq, buffer, singular, aspectFunction) {
        if (!isDraftEnabled(fq)) return
        const singularDraft = `${singular}Draft`
        // add singular draft
        buffer.addIndentedBlock(`export class ${singularDraft} extends ${aspectFunction}(__.DraftEntity) {`, () => {
            buffer.add('static readonly kind: \'entity\' | \'type\' | \'aspect\' = \'entity\'')
            this.#printStaticKeys(buffer, singularDraft)
        }, '}')

        // add plural draft
        buffer.add(`export class ${singular}Drafts extends Array<${singularDraft}> { $count?: number }`)
    }

    /**
     * @param {string} fq - fully qualified name of the entity
     * @param {string} clean - the clean name of the entity
     * @param {EntityCSN} _entity - the entity to generate the static contents for
     * @param {boolean} [isPlural] - `true` if passed entity is plural
     */
    #staticClassContents(fq, clean, _entity, isPlural = false) {
        return isDraftEnabled(fq) ? [`static drafts: typeof ${clean}Draft${isPlural ? 's' : ''}`] : []
    }
    ...
    #printEntity(fq, entity) {
        ...
        if (!isType(entity)) {
            ...
            const additionalProperties = this.#staticClassContents(fq, singular, entity, true)
