...
class Visitor {
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        buffer.add(`export class ${clean} extends ${identAspect(toLocalIdent({clean, fq}))}(${baseDefinitions.path.asIdentifier()}.Entity) {${this.#staticClassContents(clean, entity).join('\n')}}`)
        this.contexts.pop()
    }
     ...
     * @param {[string, EntityCSN][]} params - parameter list as found in CSN.
