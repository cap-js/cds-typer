...
class Resolver {
    ...
    findPropertyAccess(p) {
        ...
        while ((!defs[qualifier] || defs[qualifier].kind !== 'entity') && parts.length) {
            qualifier += `.${parts.shift()}`
        }
        // skip forward to the last entity from left (Entity3)
        while (parts.length && defs[qualifier + `.${parts[0]}`]?.kind === 'entity') {
            qualifier += `.${parts.shift()}`
        }
        // assuming Entity3 _does_ own a property "property1", return [property1, property2]
        return parts.length && Object.hasOwn(defs[qualifier]?.elements, parts[0])
            ? parts
            : []
