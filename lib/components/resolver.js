...
class Resolver {
    ...
    untangle(fq) {
        ...
        /*if (nameParts.length > 1) {
            const csn = this.visitor.csn.xtended
            let prefix = `${ns}.${nameParts.shift()}`
            while (nameParts.length && csn.definitions[prefix]) {
    ...
    resolveAndRequire(element, file) {
        ...
        const [,,propertyAccess] = this.untangle(typeName) // FINDME: insert untangle here!
