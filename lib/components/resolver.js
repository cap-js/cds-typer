...
class Resolver {
    ...
    untangle(fq) {
        ...
        return [new Path(ns.split('.')), name, property]
    }
    ...
    findPropertyAccess(p) {
        ...
        // In xtended they seem to be also present, but at least distinguishable via the 'any' kind
        // (as opposed to being a fully fledged entity)
        const defs = this.visitor.csn.xtended.definitions
        /** @type {string[]} */
        const properties = []
        let qualifier = parts.join('.')
        while ((!defs[qualifier] || isCsnAny(defs[qualifier])) && parts.length) {
            properties.unshift(parts.pop())
            qualifier = parts.join('.')
        }
        this.#cachePropertyAccess(p, properties)
        return properties.join('.') === p ? [] : properties
    }
    ...
    resolveAndRequire(element, file) {
        ...
        if (typeInfo.isBuiltin === true) {
            ...
            if (toOne && toMany) {
                ...
                if (targetTypeInfo.typeInfo.isDeepRequire === true) {
                    ...
                    const { singular, plural } = targetTypeInfo.typeInfo.inflection
                    typeName =
        ...
        const target = element.target?.name ?? element.type
        if (target) {
            const [,,propertyAccess] = this.untangle(target)
    ...
    resolveNamespace(pathParts) {
        ...
        this.#caches.namespaces[fq] = result
        return result
    }
