const validIdentPattern = /^[_$a-zA-Z][$\w]*$/

/**
 * Checks if an identifier is a valid JavaScript identifier.
 * @param {string} ident - the identifier to check
 */
const isValidIdent = ident => validIdentPattern.test(ident)
 ...
 */
const normalise = ident => ident && !isValidIdent(ident)
    ? `"${ident}"`
    : ident
