...
function printEnum(buffer, name, kvs, options = {}) {
    ...
    buffer.outdent()
    buffer.add('} as const;')
    buffer.add(`${opts.export ? 'export ' : ''}type ${name} = ${[...vals].filter(Boolean).join(' | ')}`)
    buffer.add('')
}
 ...
 */
const stringifyEnumImplementation = (name, kvs) => `module.exports.${name} = { ${kvs.map(([k,v]) => `${k}: ${JSON.stringify(v ?? k)}`).join(', ')} }`
