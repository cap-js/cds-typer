 ...
 * @param {import('../printers/typescript').Printer} printer - the printer to use for generating output
 */
function printEnum(buffer, name, kvs, options = {}, doc=[], printer) {
    const opts = {...{export: true}, ...options}
    buffer.add('// enum')
    if (opts.export) buffer.add(doc)
    buffer.add(printer.printEnumConst({ name, kvs, isExported: opts.export }))
