...
function propagateInflectionAnnotations(csn) {
    ...
    for (const [name, def] of Object.entries(csn.inferred.definitions)) {
        ...
        if (singularAnno in def) delete def[singularAnno]
        if (pluralAnno in def) delete def[pluralAnno]

        const xtendedDef = csn.xtended.definitions[name]
        if (xtendedDef) {
            // transfer annotation from xtended if existing
            if (singularAnno in xtendedDef) def[singularAnno] = xtendedDef[singularAnno]
            if (pluralAnno in xtendedDef) def[pluralAnno] = xtendedDef[pluralAnno]
        }
    }
}
