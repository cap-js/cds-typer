...
class Resolver {
    ...
    inflect(typeInfo, namespace) {
        ...
        if (typeInfo.isInlineDeclaration) {
            ...
            if (plural.indexOf('.') !== -1) plural = plural.split('.').slice(-1)[0]
    ...
    resolveAndRequire(element, file) {
        ...
        if (typeInfo.isBuiltin === true) {
            ...
            if (toOne && toMany) {
                ...
                } else if (targetTypeInfo.typeInfo.isInlineEntity === true) {
                    typeName = cardinality > 1 ? toMany(targetTypeInfo.typeInfo.inflection.plural) : toOne(targetTypeInfo.typeInfo.inflection.singular)
                } else {
        ...
        if (target && !typeInfo.isDeepRequire) {
            if (typeInfo.plainName?.indexOf('.') > -1) {
                // target is obviously an inline entity
                // update inflections with proper prefix
                const nameParts = typeInfo.plainName.split('.').slice(0, -1)
                typeInfo.inflection = {
                    singular: [...nameParts, typeInfo.inflection?.singular].join('.'),
                    plural: [...nameParts, typeInfo.inflection?.plural].join('.')
                }
                typeInfo.isInlineEntity = true
            } else {
