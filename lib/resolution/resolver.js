...
const { isEntity, getMaxCardinality, isStructuredType, isRef, hasElements } = require('../csn')
...
class Resolver {
    ...
    resolveType(element, file, options) {
        ...
        } else if (isStructuredType(element)) { //(element?.elements && (options?.forceInlineStructs || !element?.type)) {
     ...
     * @param {EntityCSN | import('../typedefs').resolver.ref | string } val - the value
     * @param {TypeResolveInfo} into - see resolveType()
