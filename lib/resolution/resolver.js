...
const { deepRequire, toManyAssociation, toOneAssociation, arrayOf, compositionOfMany, compositionOfOne, key } = require('../printers/wrappers')
...
const { isEntity, getMaxCardinality, isExternal, lookUpRefType } = require('../csn')
...
class Resolver {
    ...
    inflect(typeInfo, namespace) {
        ...
        if (typeInfo.csn?.kind === 'type') {
            return {
                ...
                plural: new Identifier(arrayOf(typeInfo.plainName)),
    ...
    resolveAndRequire(element, file, options) {
        ...
        if (typeInfo.isBuiltin === true) {
            ...
            if (typeInfo.isArray === true && typeInfo.itemsType) {
                ...
                // Recursively resolve the items type. forceInlineStructs must not be propagated here:
                // a named struct used as array item type (e.g. `array of Address`) must always be
                // referenced by name and imported, never flattened into an anonymous inline struct.
                const itemsTypeResolved = this.resolveAndRequire(element.items, file, {...options, forceInlineStructs: false})
                // Use the resolved items type name
                typeName = arrayOf(itemsTypeResolved.typeName)
            } else {
                const [toOne, toMany] =
                    {
                        Association: [toOneAssociation, toManyAssociation],
                        Composition: [compositionOfOne, compositionOfMany],
                        array: [arrayOf, arrayOf]
                 ...
                 */
                    ...
                    // For inline declarations, use plain type without normalization but wrap in Array for many cardinality
                        ...
                        if (targetTypeInfo.typeInfo.isInlineDeclaration && cardinality > 1) {
                            targetTypeName = arrayOf(targetTypeName)
                        }
        ...
        if (typeInfo.isInlineDeclaration === true) {
            ...
            if (targetEntity) {
                let namespace = this.resolveNamespace(targetEntity)
                const parent = new Path(namespace.split('.'))

                // If the target resolves to the current file but is a projection/view,
                // follow the projection to its source so the enum is referenced from
                // the original namespace rather than re-declared in the service file.
                if (parent.isCwd(file.path.asDirectory())) {
                    const targetDef = this.csn.definitions[targetEntity]
                    const sourceRef = targetDef?.query?.from?.ref?.[0] ?? targetDef?.projection?.from?.ref?.[0]
                    if (sourceRef) {
                        const sourceNamespace = this.resolveNamespace(sourceRef)
                ...
                const resolvedParent = new Path(namespace.split('.'))
                if (!resolvedParent.isCwd(file.path.asDirectory())) {
                    // Add import for the target entity's namespace
                    file.addImport(resolvedParent)
                    const parentNamespaceIdent = resolvedParent.asIdentifier()

                    // Skip if the inline enum resolver (Part 3) already qualified the type
                    // with this namespace prefix to avoid double-prepending.
                    if (!typeName.startsWith(`${parentNamespaceIdent}.`)) {
    ...
    resolveType(element, file, options) {
        ...
        if (element?.type === undefined && !element?.items) {
            ...
            if (element.parent?.name) {
                ...
                // When the entity is scoped (e.g. Status.texts), prefix with the scope so the reference
                // matches the top-level exported name produced by addInlineEnum (e.g. Status_text_code).
                const parentInfo = this.visitor.entityRepository.getByFq(element.parent.name)
                const singularName = util.singular4(element.parent, true)
                const scopedName = parentInfo?.scope?.length
                    ? [...parentInfo.scope, singularName].join('_')
                    : singularName
                const cleanEntityName = new Identifier(scopedName).normalised.plain
     ...
     * }
     * ```
     * results in `B.x` having `{ ref: [A,x] }`, but `C.x` only has a string `'A.x'` as type.
     ...
     * @param {string[]} ref - the type reference to resolve transitively
     */
    resolveTransitive (ref) {
        let resolvedType = /** @type {string} */(lookUpRefType(this.csn, ref)?.type)
        try {
            for (let i = 0; i < MAX_TRANSITIVE_RESOLUTION_STEPS; i++) {
                const { csn } = this.resolveTypeName(resolvedType)
                resolvedType = csn.type
            }
        } catch { /* ignore */ }
        return resolvedType
    }
