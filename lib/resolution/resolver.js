...
const { isBuiltinType, isInlineDeclaration } = require('./type-guards')
...
class Resolver {
    ...
    inflect(typeInfo, namespace) {
        ...
        if (typeInfo.csn?.kind === 'type') {
            const plainName = typeInfo.plainName ?? typeInfo.type ?? ''
            return {
                singular: plainName,
                plural: createArrayOf(plainName),
                typeName: plainName,
            }
        }
        ...
        if (isInlineDeclaration(typeInfo)) {
            ...
            this.structuredInlineResolver.printInlineType({
                ...
                buffer: into,
                statementEnd: '',
                modifiers: typeInfo.csn ? getPropertyModifiers(typeInfo.csn) : []
            })
            typeName = into.join()
            singular = typeName
            ...
            const csn = typeInfo.csn ?? { kind: '', type: '', name: typeInfo.plainName ?? '' }
            plural = util.getPluralAnnotation(csn) ?? typeInfo.plainName ?? ''
            // remove leading entity name
            if (plural?.includes('.')) plural = last(plural)
            singular = util.getSingularAnnotation(csn) ?? util.singular4(csn, true) // util.singular4(typeInfo.csn, true)  // can not use `plural` to honor possible @singular annotation
    ...
    resolveAndRequire(element, file, options) {
        ...
        if (isBuiltinType(typeInfo)) {
            ...
            if (toOne && toMany) {
                 ...
                 * @param {Record<string, unknown>} el - the element to check
                 * @param {string} property - the property to check
                 * @returns {import('../typedefs').resolver.EntityCSN | { type: string }}
                 */
                const getTarget = (el, property) => typeof el[property] === 'string'
                    ? { type: /** @type {string} */ (el[property]) }
                    : /** @type {import('../typedefs').resolver.EntityCSN} */ (el[property])
                ...
                if (targetTypeInfo.typeInfo.isDeepRequire === true) {
                    ...
                    const inflection = targetTypeInfo.typeInfo.inflection ?? { singular: '', plural: '' }
                    let { singular, plural } = inflection
                    // For self-reference check, we need to check the original element, not the target object
                    const isSelf = typeof target === 'string' && this.visitor.isSelfReference(target)
                    typeName = cardinality > 1
                        ? toMany(plural)
                        : toOne(isSelf ? 'this' : singular)
        ...
        const targetName = typeof element.target === 'string'
            ? element.target
            : (element.target && typeof element.target === 'object' && 'name' in element.target
                ? /** @type {{name?: string}} */ (element.target).name
                : undefined)
        const typeRef = typeof element.type !== 'string' ? element.type?.ref?.join('.') : undefined
        const target = targetName ?? typeRef ?? element.type
        const targetStr = typeof target === 'string' ? target : (typeof target === 'object' && 'ref' in target ? target.ref.join('.') : '')
        if (targetStr && !typeInfo.isDeepRequire) {
            const { propertyAccess, scope } = this.visitor.entityRepository.getByFq(targetStr) ?? {}
            if (scope?.length && typeInfo.inflection) {
                ...
                const elementName = targetStr.slice(0, -propertyAccess.join('.').length - 1)
                ...
                typeName = deepRequire(util.singular4({ name: elementName })) + access
                typeInfo.isDeepRequire = true
            }
        }
    ...
    resolveType(element, file, options) {
        ...
        if (element?.type === undefined) {
            ...
            if (element.parent?.name) {
                ...
                const elementType = typeof element.type === 'string' ? element.type : element.type?.ref?.[0] ?? ''
                if (elementType) {
                    this.resolveByTypeName(elementType, result)
                }
            }
        } else {
        ...
        if (!isBuiltinType(result) && !isInlineDeclaration(result) && !result.plainName) {
     ...
     * @param {TypeResolveInfo} into - see {@link resolveType}
    ...
    resolvePotentialReferenceType(val, into, file) {
        ...
        if (val.elements) {
            ...
            this.resolveByTypeName(val.ref[0], into)
            into.isForeignKeyReference = true
        } else {
            // val is string
            this.resolveByTypeName(val, into)
        }
    }
     ...
     * @param {string} fq - fully qualified type, like cds.String, or a.b.c.d.Foo
     * @param {TypeResolveInfo} [into] - optional dictionary to fill by reference, see {@link resolveType}
     * @returns {TypeResolveInfo} see {@link resolveType}
     */
    resolveByTypeName(fq, into) {
        ...
        if (builtin === undefined) {
            ...
            throw new Error(`Can not resolve apparent builtin type '${fq}' to any CDS type.`)
        } else if (builtin !== false) {
            // builtin
            result.type = builtin
            result.isBuiltin = true
        } else if (fq in this.csn.definitions) {
            // user-defined type
            result.type = this.trimNamespace(util.singular4(this.csn.definitions[fq])) //(path[path.length - 1])
            result.isBuiltin = false
            result.path = new Path(path) // FIXME: relative to current file
            result.csn = this.csn.definitions[fq]
            result.plainName = this.trimNamespace(fq)
        } else if (fq === '$self') {
            result.type = 'this'
            result.isBuiltin = true
            ...
            const lib = this.libraries.find(lib => lib.offers(fq))
            if (lib) {
                ...
                result.csn = { name: fq, kind: 'entity', type: fq }
                result.plainName = plain
            } else {
                throw new Error(`Can not resolve '${fq}' to any builtin, library-, or user defined type.`)
            }
        }
