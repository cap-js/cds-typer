...
class Resolver {
    ...
    inflect(typeInfo, namespace) {
        ...
        if (typeInfo.csn?.kind === 'type') {
            const { normalised } = normalise(typeInfo.plainName)
            return {
                singular: normalised,
                plural: createArrayOf(normalised),
                typeName: normalised,
                normalised: {}
            }
        }
        ...
        if (typeInfo.isInlineDeclaration) {
            ...
            singular = util.getSingularAnnotation(typeInfo.csn) ?? util.singular4(plural, true) // util.singular4(typeInfo.csn, true)  // can not use `plural` to honor possible @singular annotation
        ...
        if (typeInfo.isBuiltin === true) {
            ...
            if (toOne && toMany) {
                ...
                if (targetTypeInfo.typeInfo.isDeepRequire === true) {
                    ...
                    typeName = cardinality > 1
                        ? toMany(normalise(plural).normalised)
                        : toOne(this.visitor.isSelfReference(target)
                            ? 'this' 
                            : singular)
        ...
        if (target && !typeInfo.isDeepRequire) {
            ...
            if (scope?.length && typeInfo.inflection) {
                ...
                typeInfo.inflection = {
                    ...
                    normalised: { plural: normalise(plural) }
                }
                ...
                typeName = deepRequire(normalise(util.singular4(element)).normalised) + access
                typeInfo.isDeepRequire = true
            }
        }
