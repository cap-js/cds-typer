...
class Resolver {
    ...
    inflect(typeInfo, namespace) {
        ...
        // guard: types don't get linguistically inflected, but do honour explicit @singular/@plural annotations
        if (typeInfo.csn?.kind === 'type') {
            const singularName = util.getSingularAnnotation(typeInfo.csn) ?? typeInfo.plainName
            const pluralName = util.getPluralAnnotation(typeInfo.csn)
            return {
                singular: new Identifier(singularName),
                plural: pluralName ? new Identifier(pluralName) : new Identifier(createArrayOf(singularName)),
