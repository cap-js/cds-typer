...
class ImplementationPrinter extends TypeScriptPrinter {
    ...
    printAspect(ctx) {
        const chain = ctx.ancestorInfos
            .toReversed()
            .reduce((wrapped, ancestor) => `${asAspectFnIdentifier({info: ancestor, wrapper: ctx.identAspect, relative: ctx.filePath})}(${wrapped})`, 'Base')
...
class DeclarationPrinter extends TypeScriptPrinter {
    ...
    printAspect(ctx) {
        const instanceType = ctx.ancestorInfos
            .toReversed()
            .reduce(
                (inner, ancestor) => `ReturnType<typeof ${asAspectFnIdentifier({info: ancestor, wrapper: ctx.identAspect, relative: ctx.filePath})}<${inner}>>`,
                'TBase'
            )
        return [
            'new (...args: any[]): {',
