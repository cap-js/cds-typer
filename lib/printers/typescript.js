...
const { asClassIdentifier, asAspectFnIdentifier } = require('../resolution/entity')
...
class TypeScriptPrinter {
    ...
    stringifyFunctionParams({params, file, resolver, inlineDeclarationResolver}) {
        ...
        try {
            return this.#stringifyFunctionParams(params, file)
        } finally {
            this.resolver = undefined
            this.inlineDeclarationResolver = undefined
        }
    }
     ...
     * @returns {string[]}
     */
    getActionModifiers () {  throw new Error('not implemented') }
    ...
    printStaticActions({entity, clean, ancestors, file, resolver, inlineDeclarationResolver}) {
        ...
        this.inlineDeclarationResolver = inlineDeclarationResolver
        try {
            const prefix = `${this.getActionModifiers().join(' ')} `

            if (actions.length) {
                const actionsBuffer = new Buffer()
                for (const [aname, action] of actions) {
                    ...
                    actionsBuffer.addIndentedBlock(opener, content, closer)
                }
                return [
                    `${prefix}actions: ${intersectionOf(...inherited, '{')}`,
                    ...indent(actionsBuffer.parts, 2),
                    '};',
                ]
            } else {
                return [`${prefix}actions: ${intersectionOf(...inherited, empty)};`]
            }
        } finally {
            this.resolver = undefined
            this.inlineDeclarationResolver = undefined
...
class ImplementationPrinter extends TypeScriptPrinter {
    getActionModifiers () {
        return ['declare', 'static', 'readonly' ]
    }
    ...
    printAspect(ctx) {
        const chain = (ctx.ancestorInfos ?? [])
            .toReversed()
            .reduce((wrapped, ancestor) => `${asAspectFnIdentifier({info: ancestor, wrapper: ctx.identAspect ?? (n => `_${n}Aspect`), relative: ctx.filePath})}(${wrapped})`, 'Base')
        ...
        const classBody = [props, statics, keysEtc]
            ...
            .join('\n')
        return [
            `return class ${ctx.clean} extends ${chain} {`,
            ...indent(classBody, 2),
            '};'
        ]
    }
    ...
    /** @type {TypeScriptPrinter['printEvent']} */
    printEvent({entityName, body}) {
...
class DeclarationPrinter extends TypeScriptPrinter {
    ...
    /** @type {TypeScriptPrinter['printAspect']} */
    printAspect(ctx) {
        const instanceType = (ctx.ancestorInfos ?? [])
            .toReversed()
            .reduce(
                (inner, ancestor) => `ReturnType<typeof ${asAspectFnIdentifier({info: ancestor, wrapper: ctx.identAspect ?? (n => `_${n}Aspect`), relative: ctx.filePath})}<${inner}>>`,
                'TBase'
            )
        return [
            'new (...args: any[]): {',
            ...indent(ctx.properties?.parts ?? [], 2),
            `} & InstanceType<${instanceType}>`,
            ...indent(ctx.staticMembers?.parts ?? [], 2),
            ...indent(ctx.keysElementsActions?.parts ?? [], 2),
        ]
    }
