...
class TypeScriptPrinter {
     ...
     * @param {[string, string][]} kvs - key-value pairs
     * @param {boolean} isExported - whether the enum should be exported
     * @returns {string} the const declaration
     */
    // eslint-disable-next-line no-unused-vars
    printEnumConst(name, kvs, isExported) {
        throw Error('not implemented')
    }
...
class ImplementationPrinter extends TypeScriptPrinter{
    ...
    printEnumConst(name, kvs, isExported) {
        ...
        const entries = kvs.map(([k, v]) => `${enquote(k)}: ${v}`).join(', ')
        return `${isExported ? 'export ' : ''}const ${name} = { ${entries} } as const;`
    }
}
...
class DeclarationPrinter extends TypeScriptPrinter {
    ...
    printEnumConst(name, kvs, isExported) {
        const { enquote } = require('../components/identifier')
        const entries = kvs.map(([k, v]) => `${enquote(k)}: ${v}`).join(',\n  ')
        return `${isExported ? 'export ' : ''}const ${name}: {\n  ${entries},\n}`
    }
}
