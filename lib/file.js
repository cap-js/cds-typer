...
class SourceFile {
    ...
    addAction(name, params, returns) {
        ...
        this.actions.add(`export declare const ${name}: (${ps}) => ${returns};`)
    }
    ...
    toJSExports() {
        return [AUTO_GEN_NOTE, "const cds = require('@sap/cds')", `const cson = cds.entities('${this.path.asNamespace()}')`] // boilerplate
            .concat(
                this.inflections
                    ...
                    .flatMap(([singular, plural, original]) => Array.from(new Set([
                        `module.exports.${singular} = cson.${original}`,
                        `module.exports.${plural} = cson.${original}`,
                        `module.exports.${original} = cson.${original}`
                    ])))
