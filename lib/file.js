...
const { createObjectOf, stringIdent } = require('./components/wrappers')
...
class SourceFile extends File {
    ...
    #getJSExportBoilerplate() {
        ...
        if (configuration.useEntitiesProxy) {
            if (namespace === '_') {
                boilerplate.push(this.jsPrinter.printImport('cds', '@sap/cds'), this.#getEntityProxyFunctionExport())
            } else {
                boilerplate.push(`const { createEntityProxy } = require('${new Path(['_']).asDirectory({relative: this.path.asDirectory()})}')`)
            }
        } else {
            boilerplate.push(
                this.jsPrinter.printImport('cds', '@sap/cds'),
                this.jsPrinter.printConstant('csn', `cds.entities('${namespace}')`)
            )
        }
        return boilerplate
    }
            ...
            .concat(
                ...
                this.services.names.flatMap(name => {
                    ...
                    return [
                        ...
                        jsp.printExport(nameSimple, nameSimple)
                    ]
                })
            )
