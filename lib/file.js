...
const { enquote, Identifier } = require('./components/identifier')
const { empty } = require('./components/typescript')
const { proxyAccessFunction } = require('./components/javascript')
const { objectOf, stringIdent, docify } = require('./printers/wrappers')
...
class SourceFile extends File {
     ...
     * @param {import('./printers/typescript').Printer} [tsPrinter] - optional TypeScript printer
     */
    constructor(path, tsPrinter) {
        super()
        /** @type {Path} */
    ...
    static stringifyLambda({name, parameters=[], returns='any', kind, initialiser, self='never', isStatic=false, callStyles={positional:true, named:true}, doc}) {
        ...
        if (callStyles.named) {
            const parameterNames = objectOf(parameters.map(({name}) => enquote(name)).join(', '))
    ...
    addEnum(fq, name, kvs, doc) {
        ...
        if (!this.#tsPrinter) throw new Error('TypeScript printer not set for SourceFile')
        printEnum(this.enums.buffer, name, kvs, {}, doc, this.#tsPrinter)
    }

    /**
     * Adds an inline enum to this file.
     * @param {object} options - options
     * @param {string} options.entityCleanName - name of the entity the enum is attached to without namespace
     * @param {string} options.entityFqName - name of the entity the enum is attached to with namespace
     * @param {string} options.propertyName - property to which the enum is attached.
     * @param {[string, string][]} options.kvs - list of key-value pairs
     * @param {Buffer} [options.buffer] - if buffer is of subnamespace the enum will be added there,
     *                                    otherwise to the inline enums of the file
     * @param {string[]} [options.doc] - the enum docs
     * @param {boolean} [options.skipTsDeclaration] - when true, registers the enum in the JS output
     *  (so `module.exports.Entity.prop = { ... }` is still emitted) but suppresses the TypeScript
     *  `const`/`type` declaration. Use this for projected entities whose enum is already declared
     *  in the source namespace file and should be referenced from there rather than re-declared.
     * @param {boolean} [options.skipJsRegistration] - when true, suppresses the JS `Entity.prop ??= {...}`
     *  assignment entirely. Use this for pure CDS aspects, which have no runtime JS value and therefore
     *  cannot be used as an LHS in the assignment.
     * @example
     * ```js
     * addInlineEnum({ entityCleanName: 'Books', entityFqName: 'Books', propertyName: 'genre', kvs: [['horror','horror']] })
     * ```
     * generates
     * ```js
     * // index.js
     ...
     * ```
     */
    addInlineEnum({ entityCleanName, entityFqName, propertyName, kvs, buffer, doc=[], skipTsDeclaration=false, skipJsRegistration=false }) {
        const parentSegments = new Identifier(entityFqName)
            .scope
            .slice(this.path.parts.length)
        const namespacedEntity = [...parentSegments, entityCleanName].join('.')
        if (!skipJsRegistration) {
        ...
        // For projected entities, the enum is declared in the source namespace file.
        // We still register the enum data above (for JS output), but skip the TS declaration.
        if (skipTsDeclaration) return

        if (!this.#tsPrinter) throw new Error('TypeScript printer not set for SourceFile')
        const printer = this.#tsPrinter

        // REVISIT: find a better way to do this???
        if (buffer?.namespace) {
            const printEnumToBuffer = (/** @type {Buffer} */buffer) => printEnum(buffer, propertyToInlineEnumName(entityCleanName, propertyName), kvs, {export: false}, doc, printer)
            ...
            // When the entity is scoped (e.g. Status.texts lives in namespace Status), prefix the
            // exported name with the scope to avoid collisions between sibling scoped entities that
            // share the same singular name (e.g. Status.texts and Urgency.texts both become "text").
            const scopedEntityName = parentSegments.length
                ? [...parentSegments, entityCleanName].join('_')
                : entityCleanName
            const printEnumToBuffer = (/** @type {Buffer} */buffer) => printEnum(buffer, propertyToInlineEnumName(scopedEntityName, propertyName), kvs, {export: true}, doc, printer)
...
class Buffer {
     ...
     * @param {string | string[] | (() => string) | ((() => string) | string)[]} part  - what to attach to the buffer
     */
    add(part) {
...
class FileRepository {
    ...
    getNamespaceFile(path) {
        ...
        return (this.#files[key] ??= new SourceFile(path, this.#tsPrinter))
    }
        ...
        sources.map(async source => {
            ...
            try {
                ...
                if (err instanceof Error) {
                    LOG.error(`Could not create parent directory ${dir}: ${err}.`)
                    LOG.error(err.stack)
                } else {
                    LOG.error(`Could not create parent directory ${dir}: ${String(err)}`)
                }
            }
            return dir
        })
