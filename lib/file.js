...
const { isDraftEnabled } = require('./csn')
...
class SourceFile extends File {
    ...
    /**
     * Returns exports for draft entities or an empty array if the entity
     * is not draft enabled
     * @param {string} singular - singular name of entity
     * @param {string} original - original name of entity
     * @returns {string[]} exports with singular/plural of draft
     */
    #getDraftEntityJsExports(singular, original) {
        const namespace = this.path.asNamespace()
        if (!isDraftEnabled(`${namespace}.${original}`)) return []

        const comment = `// ${original}.drafts `

        if (configuration.useEntitiesProxy) {
            return [
                comment,
                `module.exports.${singular}Draft = createEntityProxy(['${namespace}', '${original}', 'drafts'], { target: { is_singular: true } })`,
                `module.exports.${singular}Drafts = createEntityProxy(['${namespace}', '${original}', 'drafts'])`
            ]
        } else {
            return [
                comment,
                `module.exports.${singular}Draft ={ is_singular: true, __proto__: csn.${original}.drafts }`,
                `module.exports.${singular}Drafts = csn.${original}.drafts`
            ]
        }
    }
    toJSExports() {
        return this.#getJSExportBoilerplate() // boilerplate
            ...
            // "module.exports.Books" is defined before "module.exports.Books.text"
                ...
                .flatMap(([singular, plural, original]) => {
                    ...
                    exports.push(...this.#getDraftEntityJsExports(singular, original))
                    return exports
                })
