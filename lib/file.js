...
class SourceFile extends File {
     ...
     * @param {[string, string, string, string?][]} [options.parameters] - list of parameters, passed as [name, modifier, type, doc] pairs
     ...
     * stringifyLambda({name: 'f', parameters: [['p','?','T','/** doc *\/']], returns: 'number'})  // /** doc *\/f?: { (p: T) => number, ...  }
    ...
    static stringifyLambda({name, parameters=[], returns='any', kind, initialiser, isStatic=false, callStyles={positional:true, named:true}, doc}) {
        let docStr = doc?.length === 1 ? doc.join('\n')+'\n' : ''
        const parameterTypes = parameters.map(([n, mod='', t, doc='']) => `\n${doc}${normalise(n)}${mod}: ${t}`).join(', ')
        ...
        if (callStyles.positional) {
            const paramTypesPositional = parameters.map(([n, , t, doc='']) => `\n${doc}${normalise(n)}: ${t}`).join(', ') // must not include ? modifiers
            callableSignatures.push(`// positional\n${docStr}(${paramTypesPositional}): ${returns}`)
        }
        if (callStyles.named) {
     ...
     * @param {[string, string, string, string][]} parameters - list of parameters, passed as [name, modifier, type] tuple
