    ...
    getImports() {
        ...
        for (const imp of Object.values(this.imports)) {
            if (!imp.isCwd(this.path.asDirectory())) {
                const importSuffix = configuration.targetModuleType === 'esm' ? '/index.js' : ''
                buffer.add(`import * as ${imp.asIdentifier()} from '${imp.asDirectory({relative: this.path.asDirectory()})}${importSuffix}';`)
            }
        }
        buffer.blankLine()
        return buffer
