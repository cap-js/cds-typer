...
const dir = './files'
const ccs = path.normalize('./' + path.join(dir, 'cloud-cap-samples'))
const output = path.normalize(`${dir}/output`)
...
const check = async (file, resolveAlias = false) => {
    ...
    const paths = await cds2ts
        .compileFromFile(path.normalize(file), { rootDirectory: output, logLevel: Levels.NONE })
